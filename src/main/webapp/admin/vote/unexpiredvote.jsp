	 ...
	 <c:if test="${voteitemQueryVo.list != null && fn:length(voteitemQueryVo.list)>0}">
	 	<c:forEach items="${voteitemQueryVo.list}" varStatus="s" var="v">
	  	<tr align="center">
	   		...
	   		<td><a href="${pageContext.request.contextPath}/voteItem/deleteVote?vid=${v.vid}&page=${voteitemQueryVo.newpage
	   		}"><img src="${pageContext.request.contextPath}/image/i_del.gif"/></a></td>
	 	</tr>
	  </c:forEach>
	 </c:if>
  <tr align="center">
  	<c:if test="${voteitemQueryVo.pagemark == 8 }">
  		<td colspan="9">
	  		<a href="${pageContext.request.contextPath}/voteItem/findVoteByUnexpired?page=1">首页</a>&nbsp;
	  		<c:if test="${voteitemQueryVo.newpage > 1}">
	  			<a href="${pageContext.request.contextPath}/voteItem/findVoteByUnexpired?page=${voteitemQueryVo.newpage-1}">上一页</a>&nbsp;
	  		</c:if>
	  		<c:if test="${voteitemQueryVo.newpage <= 1}">
	  			<a href="${pageContext.request.contextPath}/voteItem/findVoteByUnexpired?page=1">上一页</a>&nbsp;
	  		</c:if>
	  		${voteitemQueryVo.newpage}/${voteitemQueryVo.pagecount}&nbsp;
	  		<c:if test="${voteitemQueryVo.newpage < voteitemQueryVo.pagecount}">
	  			<a href="${pageContext.request.contextPath}/voteItem/findVoteByUnexpired?page=${voteitemQueryVo.newpage + 1}">下一页</a>&nbsp;
	  		</c:if>
	  		<c:if test="${voteitemQueryVo.newpage >= voteitemQueryVo.pagecount && voteitemQueryVo.pagecount != 0}">
	  			<a href="${pageContext.request.contextPath}/voteItem/findVoteByUnexpired?page=${voteitemQueryVo.pagecount}">下一页</a>&nbsp;
	  		</c:if>
	  		<c:if test="${voteitemQueryVo.newpage >= voteitemQueryVo.pagecount && voteitemQueryVo.pagecount == 0}">
	  			<a href="${pageContext.request.contextPath}/voteItem/findVoteByUnexpired?page=1">下一页</a>&nbsp;
	  		</c:if>
	  		<c:if test="${voteitemQueryVo.pagecount != 0}">
	  			<a href="${pageContext.request.contextPath}/voteItem/findVoteByUnexpired?page=${voteitemQueryVo.pagecount}">尾页</a>
	  		</c:if>
	  		<c:if test="${voteitemQueryVo.pagecount == 0}">
	  			<a href="${pageContext.request.contextPath}/voteItem/findVoteByUnexpired?page=1">尾页</a>
	  		</c:if>
  		</td>
  	</c:if>
